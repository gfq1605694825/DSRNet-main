...
class Eval_thread():
    def __init__(self, loader, dataset, output_dir, cuda):
        self.loader = loader
    ...
    def run(self):
        ...
        self.LOG(
            ' ({:^12}):{:.4f}{:^6}||{:.4f} max-fm {:^6}|| {:.4f} max-Emeasure {:^6}|| {:.4f} {:^6} || {:.4f} mean-Emeasure || {:.4f} mean-fm || {:.4f} AP || {:.4f} AUC.\n'
            .format(self.dataset, mae, "mae", max_f, "max-fm", max_e, "max_e", s, "S-m",
                    mean_e, mean_f, avg_p, auc))
        print(
            ' ({:^12}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP || {:.4f} AUC.\n'
            .format(self.dataset, mae, max_f, mean_f, max_e,
                    mean_e, s, avg_p, auc))
        return '[cost:{:.4f}s]  ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP || {:.4f} AUC.'.format(
            time.time() - start_time, self.dataset, mae, max_f,
            mean_f, max_e, mean_e, s, avg_p, auc)

    def Eval_mae(self):
        print('eval[MAE]:{} dataset.'.format(
            self.dataset))
    ...
    def Eval_fmeasure(self):
        print('eval[FMeasure]:{} dataset'.format(
            self.dataset))
        beta2 = 0.3
    ...
    def Eval_Emeasure(self):
        print('eval[EMeasure]:{} dataset'.format(
            self.dataset))
    ...
    def Eval_Smeasure(self):
        print('eval[SMeasure]:{} dataset'.format(
            self.dataset))
