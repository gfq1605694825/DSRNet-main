...
if __name__ == '__main__':
    ...
    a = np.random.random((1, 3, 320, 320))
    b = np.random.random((1, 64, 56, 56))
    bb = np.random.random((1, 128, 28, 28))
    c = torch.Tensor(a).cuda()
    d = torch.Tensor(b).cuda()
    dd = torch.Tensor(bb).cuda()
    swinNet = p2t_base().cuda()
    swinNet.load_state_dict(torch.load(pre_path))
    # swinNet.load_pre(".")
    c1, c2, c3, c4 = swinNet(c)
    print(c1.shape)
    print(c2.shape)
    print(c3.shape)
    print(c4.shape)
