...
class Augment:
    ...
    def crop_edge(self, MASK, IMG, edge):
        ...
        if dice < .1 and w > self.psize and h > self.psize:
            ...
            crop_MASK = MASK[new_h:new_h + self.psize, new_w:new_w + self.psize]
            crop_IMG = IMG[new_h:new_h + self.psize, new_w:new_w + self.psize, :]
            crop_edge = edge[new_h:new_h + self.psize, new_w:new_w + self.psize]
            # crop_IMG = IMG[new_h:new_h + self.psize, new_w:new_w + self.psize, :]
        else:
            # crop_IMG = cv2.resize(IMG, (self.psize, self.psize), interpolation=cv2.INTER_LINEAR)
            ...
            crop_edge = cv2.resize(edge, (self.psize, self.psize), interpolation=cv2.INTER_LINEAR)
            crop_edge = (np.array(crop_edge) > 127).astype(np.float64)

        return crop_MASK.copy(), crop_IMG.copy(), crop_edge.copy()
    ...
    def flip_and_rotate_edge(self, MASK, IMG, edge):
        # h,w,c
        hflip = random.random() < 0.5
        # vflip = random.random() < 0.5
        rot90 = random.random() < 0.5
        ...
        return MASK.copy(), IMG.copy(), edge.copy()
